...
def test_the_daily_payload_survives_shrink():
    """shrink() now counts tokens against the kind's own completion cap
    (PR #51's budget guard), so the daily is sized against 1,200 rather than
    the weekly's 6,000. A one-claim day fits either way and must come
    through whole."""
    from pipeline.weekly import MAX_TOKENS, gather_daily, shrink
