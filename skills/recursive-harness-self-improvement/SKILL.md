...
description: Evidence-backed method for the loop in which an agent reads its own failed runs, rewrites its own scaffold unattended, and keeps or reverts each edit on measured evidence. The editor here is the agent, not a person, and no weights are trained. Use when an automated loop edits its own prompts, tool code or memory between runs and keeps whatever measures better; when unattended self-edits raise the score on the tasks the loop evolves against while real work does not improve; when a self-evolving system overfits the cases it evolved on; when choosing the gate that accepts or reverts a proposed edit, or how much one edit may touch; when attributing an observed failure to one function inside a long execution log before editing anything; when the loop's own diagnosis and evaluation cost more than its edits gain; when several automated workers rewrite one shared repository and converge on a single idea; or when an unattended run reports a win and you have to judge whether that win is real.
version: 1
status: active
provenance:
