...
description: Evidence-backed method for deciding whether an evaluation can be trusted when the instrument is itself a model or a generated artifact: a rubric a model wrote, a benchmark a pipeline assembled, an LLM judge, or a pass-or-fail verdict standing in for correctness. This is about the measuring instrument, not about the system being measured. Use when writing or accepting a model-written rubric or grading prompt, when an agent finds a shortcut that satisfies the grader without doing the work, when candidates pass the tests for the wrong reason, when a judge is trained or prompted on other judgments and its scores start clustering, when benchmark items turn out to be ambiguous or wrong, when deciding how many independent judgments an accept should require, when a measured gain might be noise or leakage, when a single-turn evaluation promises reliability that multi-turn use does not deliver, or when auditing a result that someone else reports.
version: 1
status: active
provenance:
